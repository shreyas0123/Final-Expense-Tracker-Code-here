...
//It's trying to find the container where you're displaying your list of items means we need to get list of expenses added 
//container basically its a box where you can put other things inside. In our code contaner will hold the added expense items in it
const itemListContainer = document.getElementById('listOfItems'); 

//while (itemListContainer.firstChild) is a loop that says: "While there is still a child inside the container..."
while (itemListContainer.firstChild) {
    
    //Inside the loop, itemListContainer.removeChild(itemListContainer.firstChild) says: "Remove the first child from the container."
    itemListContainer.removeChild(itemListContainer.firstChild);
}
//showing only current page expense
//from 31 to 40 line explanation:
//earlier what happens means when if there is an next page exist if i click on next page button previous page expense along with current page expense will be displaying
//but i need when i click on next page button i only need to get that page expense(current page) so here previous page expenses were removed 
//similalry when i click on previous page button im getting previous page expense only(current page)
    ...
    await axios.get(`http://localhost:3000/get-expense?page=${page}&pagesize=${pagesize}`, {
